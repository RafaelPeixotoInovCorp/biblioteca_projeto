<div class="p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-3xl font-bold text-base-content">Autores</h2>
        ...
        @forelse($autores as $autor)
            <div class="card bg-base-100 shadow-xl hover:shadow-2xl transition-all duration-300">
                            ...
                            <div class="w-32 rounded-xl">
                                ...
                                @else
                                    <div class="bg-primary/20 w-full h-full flex items-center justify-center">
                                        <span class="text-4xl font-bold text-primary">{{ substr($autor->nome, 0, 1) }}</span>
                                    </div>
                                @endif
                            </div>
                        </div>
                ...
                <div class="card-body items-center text-center">
                    <a href="{{ route('livros.index', ['filtro_autor' => $autor->id]) }}" class="hover:text-primary transition-colors">
                        <h3 class="card-title text-xl">{{ $autor->nome }}</h3>
                    </a>

                    <p class="text-base-content/70">
                        {{ $autor->livros->count() }} {{ $autor->livros->count() == 1 ? 'livro' : 'livros' }}
                    </p>

                    <div class="card-actions justify-center mt-2">
                        <a href="{{ route('livros.index', ['filtro_autor' => $autor->id]) }}" class="btn btn-sm btn-outline btn-primary">
                    ...
                    @if($canManage)
                        <div class="card-actions justify-center mt-2 pt-2 border-t border-base-200 w-full">
                            <a href="{{ route('admin.autores.editar', $autor->id) }}" class="btn btn-sm btn-info">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
