<nav x-data="{ open: false }" class="bg-base-100 border-b border-base-300 shadow-sm relative z-10">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="flex items-center gap-8">
                ...
                <div class="hidden sm:flex sm:items-center space-x-1">
                    ...
                    <a href="{{ route('carrinho.index') }}" class="btn btn-ghost btn-circle relative">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                        </svg>
                        @if(($carrinho = \App\Models\Carrinho::where('user_id', auth()->id())->first()) && $carrinho->itens->count() > 0)
                            <span class="badge badge-primary badge-sm absolute -top-1 -right-1">{{ $carrinho->itens->count() }}</span>
                        @endif
                    </a>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="sm:hidden">
        <div class="pt-2 pb-3 space-y-1 border-t border-base-200">
            ...
            <a href="{{ route('encomendas.index') }}"
                class="block px-4 py-2 text-base hover:bg-base-200 transition-colors duration-150 ease-in-out
                    {{ request()->routeIs('encomendas.*') ? 'bg-primary/10 text-primary font-medium' : 'text-base-content/70' }}">
                Encomendas
            </a>
