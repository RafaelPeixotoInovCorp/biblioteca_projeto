<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-base-content leading-tight">
            {{ isset($autor) ? 'Editar Autor' : 'Novo Autor' }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-base-100 overflow-hidden shadow-xl sm:rounded-lg p-6">
                <form action="{{ isset($autor) ? route('admin.autores.update', $autor->id) : route('admin.autores.store') }}"
                      method="POST"
                    ...
                    <div class="flex justify-end gap-4 mt-8">
                        ...
                        <button type="submit" class="btn btn-primary">
                            {{ isset($autor) ? 'Atualizar Autor' : 'Criar Autor' }}
                        </button>
                    </div>
                </form>
            </div>
